  ...
  module FrontendHelper
    ...
    def link_to_cart(text = nil)
      ...
      if simple_current_order.nil? or simple_current_order.item_count.zero?
        text = "#{text} <span id='cart_items_number'>0 #{Spree.t('item')}(s)</span>"
        css_class = 'empty'
      else
        text = "#{text} <span id='cart_items_number'>#{simple_current_order.item_count} #{Spree.t('item')}(s)  #{simple_current_order.display_total.to_html}</span>"
        css_class = 'full'
      end
